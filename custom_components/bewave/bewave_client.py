import socket, struct, hashlib, time, secrets
...
def _s(v): return v.decode('latin1') if isinstance(v, (bytes, bytearray)) else (v or "")
    ...
    while i < len(buf):
        try:
            ...
            if wt == 0:
                ...
                if depth < 12 and not _looks_str(sub): res += find_varint(sub, target, depth + 1)
            else: break
        except Exception: break
    return res

def armed_state(plaintext):
    if 89 not in pb_read(plaintext): return None
...
def hub_info(plaintext):
    ...
    if isinstance(pw, int) and pw > 100: pw = None
    fw = None
    if 40 in body:
...
def network_active(plaintext):
    ...
    for e in f1.get(1, []):
        ed = pb_read(e)
        if ed.get(5) and pb_read(ed[5][0]).get(2, [0])[0] == 1:
...
def _telem_from_block(f3block, f2list):
    ...
    for t in f2list:
        td = pb_read(t)
        if td.get(1, [None])[0] == 6 and 3 in td and isinstance(td[3][0], int): temp = _f32(td[3][0])
...
def parse_config_devices(plaintext):
    ...
    for room in body.get(4, []):
        ...
        for dev in rd.get(5, []):
            ...
            tel = {"type": None}
            if 20 in dd:
                t = pb_read(dd[20][0]); tel = _telem_from_block(t[3][0] if 3 in t else None, t.get(2, []))
            out[devid] = {"name": _s(dd.get(9, [b''])[0]), "room": rname,
                          "type": tel.get("type"), "model": DEV_MODELS.get(dd.get(2, [None])[0]),
                          "sysnum": dd.get(21, [None])[0], "bypass": bool(dd.get(22, [0])[0]),
                          "signal": tel.get("signal"), "battery": tel.get("battery"),
                          "state": tel.get("state"), "temp": tel.get("temp"), "volt": tel.get("volt")}
    return out
...
class BeWaveClient:
    def __init__(self, login, password, device_uuid=None, device_token=None):
        ...
        self.serial = None; self.session_id = None; self.session_secret = None; self._counter = 0
        self.K_signin = md5(password) * 2; self.A_signin = login.encode(); self.A_resp = md5(pad(login, 64))
    def next_counter(self): self._counter += 1; return self._counter
    def signin_message(self):
        inner = pb_bytes(2, self.uuid.encode()) + pb_bytes(3, (self.serial or "").encode())
        return build_message(self.next_counter(), pb_bytes(6, pb_bytes(1, inner)), self.K_signin, self.A_signin, with_ids=False)
    def command_message(self, plaintext):
        if not self.session_id: raise BeWaveError("not signed in")
    ...
    def register_plaintext(self): return pb_bytes(8, pb_bytes(1, pb_bytes(1, self.token.encode()) + pb_varint(2, 1)))
    ...
    def setting_toggle_plaintext(self, setting_id): return pb_bytes(34, pb_bytes(1, pb_varint(1, setting_id) + pb_bytes(2, pb_bytes(3, b""))))
    def decrypt(self, buf, i):
        ...
        iv = struct.pack('>III', fd.get(7, 0) & 0xffffffff, fd.get(6, 0) & 0xffffffff, fd.get(5, 0) & 0xffffffff)
        end = off + clen + 16
        for key, aad in self._decrypt_keys():
            try: return fd, AESGCM(key).decrypt(iv, ct + tag, aad), end
            except Exception: continue
        return fd, None, end
    def _decrypt_keys(self):
        ks = [(self.uuid.encode(), self.A_resp)]
        if self.session_secret and self.session_id:
            ks.append((self.session_secret, md5(self.session_id))); ks.append((self.uuid.encode(), md5(self.session_id)))
        ks.append((self.K_signin, self.A_signin)); return ks
    def ingest_response(self, plaintext):
        try:
            l2 = pb_read(pb_read(pb_read(plaintext)[7][0])[1][0])
            self.serial = l2[2][0].decode('latin1'); self.session_id = l2[3][0].decode('latin1'); self.session_secret = l2[4][0]
            return True
        except Exception: return False

class LocalConnection:
    def __init__(self, host, client, port=DATA_PORT, timeout=8):
        self.host = host; self.port = port; self.client = client; self.timeout = timeout; self.sock = None; self._buf = b''
    def connect_and_signin(self):
        ...
        while time.time() < deadline and not signed:
            ...
            for _fd, pt in self._drain():
                if pt and len(pt) > 200 and self.client.ingest_response(pt): signed = True; break
        if not signed: raise BeWaveError("sign-in response not received/decrypted")
        for f in STATE_SUBS:
            self.sock.sendall(self.client.command_message(self.client.subscribe_plaintext(f))); time.sleep(0.02)
        try: self.sock.sendall(self.client.command_message(self.client.register_plaintext()))
        except Exception: pass
        return True
    def refresh(self):
        for f in REFRESH_SUBS:
            try: self.sock.sendall(self.client.command_message(self.client.subscribe_plaintext(f)))
            except Exception: pass
    def _pump(self):
        try:
    ...
    def _drain(self):
        ...
        while i < len(self._buf):
            if self._buf[i] != 0x0a: i += 1; continue
        ...
        if len(self._buf) > 200000: self._buf = b""
        return out
    def send_command(self, plaintext): self.sock.sendall(self.client.command_message(plaintext))
    ...
    def read_messages(self, seconds=2):
        out = []; t0 = time.time()
        while time.time() - t0 < seconds: self._pump(); out += self._drain()
        return out
    def close(self):
        try: self.sock.close()
        except Exception: pass
